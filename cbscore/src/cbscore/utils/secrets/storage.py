...
def storage_get_s3_creds(
    ...
    if not entry:
        msg = f"storage secret '{host}' not found"
        logger.error(msg)
    ...
    if isinstance(entry, StoragePlainS3Secret):
        ...
        if not vault:
            msg = f"storage secret '{host}' requires vault, but no vault configured"
            logger.error(msg)
            raise ValueError(msg)
        ...
        try:
            ...
            msg = f"error retrieving storage secret '{host}' from vault: {e}"
            logger.error(msg)
