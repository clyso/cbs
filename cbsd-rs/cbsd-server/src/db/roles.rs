...
pub async fn create_role(
    ...
    sqlx::query("INSERT INTO roles (name, description, builtin) VALUES (?, ?, ?)")
        .bind(name)
        .bind(description)
        .bind(builtin as i32)
        .execute(pool)
...
pub async fn list_roles(pool: &SqlitePool) -> Result<Vec<RoleRecord>, sqlx::Error> {
    let rows =
        sqlx::query("SELECT name, description, builtin, created_at FROM roles ORDER BY name")
            .fetch_all(pool)
            .await?;
...
pub async fn delete_role(pool: &SqlitePool, name: &str) -> Result<bool, sqlx::Error> {
    ...
    let result = sqlx::query("DELETE FROM roles WHERE name = ? AND builtin = 0")
        .bind(name)
        .execute(pool)
        .await?;
...
pub async fn set_role_caps(
    ...
    sqlx::query("DELETE FROM role_caps WHERE role_name = ?")
        .bind(role_name)
        .execute(&mut *tx)
        .await?;
...
pub async fn get_role_caps(
    ...
    Ok(rows.into_iter().map(|r| r.get("cap")).collect())
}
...
pub async fn set_user_roles(
    ...
    sqlx::query("DELETE FROM user_roles WHERE user_email = ?")
        .bind(user_email)
        .execute(&mut *tx)
        .await?;

    for assignment in assignments {
        sqlx::query("INSERT INTO user_roles (user_email, role_name) VALUES (?, ?)")
            .bind(user_email)
...
pub async fn get_user_roles(
    ...
    let role_rows =
        sqlx::query("SELECT role_name FROM user_roles WHERE user_email = ? ORDER BY role_name")
            .bind(user_email)
            .fetch_all(pool)
            .await?;
    ...
    for role_row in role_rows {
        let role_name: String = role_row.get("role_name");

        let scope_rows = sqlx::query(
            "SELECT scope_type, pattern FROM user_role_scopes
...
pub async fn get_user_assignments_with_scopes(
    ...
    for role_row in role_rows {
        ...
        let cap_rows = sqlx::query("SELECT cap FROM role_caps WHERE role_name = ? ORDER BY cap")
            .bind(&role_name)
            .fetch_all(pool)
            .await?;

        let caps: Vec<String> = cap_rows.into_iter().map(|r| r.get("cap")).collect();
...
pub async fn is_role_builtin(pool: &SqlitePool, role_name: &str) -> Result<bool, sqlx::Error> {
    let row = sqlx::query("SELECT builtin FROM roles WHERE name = ?")
        .bind(role_name)
        .fetch_optional(pool)
        .await?;

    Ok(row.is_some_and(|r| r.get::<i32, _>("builtin") != 0))
}
